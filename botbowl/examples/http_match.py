...
def main():
    ...
    parser.add_argument('--creation-request-id', type=positive,
                        help='strictly increasing ID per administrator for each new session; '
                             'required for a persistent service, defaults to 1 with --loopback')
    args = parser.parse_args()
    if not args.loopback and args.creation_request_id is None:
        parser.error('--creation-request-id is required for a persistent service')
    creation_request_id = args.creation_request_id if args.creation_request_id is not None else 1
    ...
    with ExitStack() as stack:
        ...
        session = clients['admin'].create(
            request_id=creation_request_id, config=SessionConfig(size=1, max_decisions=args.max_decisions,
