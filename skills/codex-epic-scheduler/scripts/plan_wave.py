...
def _unique_list(value, field):
    ...
    try:
        ...
        raise ContractError("{} entries must be scalar values".format(field))
...
def _validate_snapshot(snapshot):
    ...
    worker_limit = _positive_integer(
        snapshot.get("max_parallel_workers"), "max_parallel_workers"
    )
    children = snapshot.get("children")
    if not isinstance(children, list):
        raise ContractError("children must be a list")
    ...
    for index, child in enumerate(children):
        ...
        dependencies = _unique_list(
            child.get("depends_on"), "{}.depends_on".format(field)
        )
...
def plan_wave(snapshot):
    ...
    worker_limit, records = _validate_snapshot(snapshot)
    active = sorted(
        issue for issue, child in records.items() if child["state"] in ACTIVE_STATES
    )
    available_slots = max(0, worker_limit - len(active))
    held_mutexes = {
        mutex
        for issue in active
        for mutex in records[issue]["mutex"]
    }
    candidates = sorted(
        issue
        for issue, child in records.items()
